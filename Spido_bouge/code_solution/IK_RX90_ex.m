...
for j=1:4
theta1(j)=atan2(PY,PX); 
end 

for j=5:8
theta1(j)=atan2(PY,PX)+pi;
end 
...
%Résolution theta3
for j=1:8 
    theta3(j)=atan2((b1(j)*cos(theta2(j))-PZ*sin(theta2(j))-a),(b1(j)*sin(theta2(j))+PZ*cos(theta2(j))));
end
...
j=3;
conf = [THETA(j,1),THETA(j,2),THETA(j,3),THETA(j,4),THETA(j,5),THETA(j,6)]';
